...
$resultSet = $dbh->query("SELECT * FROM pagos WHERE fecha between '$fechaDesde' and '$fechaHasta' order by fecha asc");
...
while($pagos = $resultSet->fetchObject()){
	...
	for ($i=0; $i < count($arrPagos); $i++) { 
		if($arrPagos[$i]->idAlumno == $pagos->idAlumno && $arrPagos[$i]->idCurso == $pagos->idCurso)
		{
