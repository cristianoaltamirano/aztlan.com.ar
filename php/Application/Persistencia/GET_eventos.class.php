...
class GET_eventos extends DA_Abs{
	...
	public static function setEvento($params){		
			...
			$lugar = $params['lugar'];
			$charla_beca = $params['charla_beca'];
			...
			if(isset($params['idEvento'])){
				...
				$sql = "INSERT INTO eventos (tipoEvento_idTipo,fecha,date,titulo,subtitulo,fechaStr,horario,maxHorario,idYoutube,link,texto,lugares,lugar_idLugar,activo,mostrarHome,mostrarAstro,mostrarPsico,charla_beca)
					VALUES ('$tipo',now(),'$date','$titulo','$subtitulo','$fechaStr','$horario','$maxHorario','$idYoutube','$link','$texto','$disponibilidad','$lugar','$activo','$mostrarHome','$mostrarAstro','$mostrarPsico','$charla_beca');";	
			}
	...
	public static function setPresentismo($params){
		...
		foreach ($params as $reserva){
			...
			$contactar = ($reserva->contactar) ? $reserva->contactar : '0';
			$contactado = ($reserva->contactado) ? $reserva->contactado : '0';
			$rechazar = ($reserva->rechazar) ? $reserva->rechazar : '0';
			$agenentrevista = ($reserva->agenentrevista) ? $reserva->agenentrevista : '0';
			$facebook = ($reserva->facebook) ? $reserva->facebook : '';
			$comentario = ($reserva->comentario) ? $reserva->comentario : '';
			
			$sql = "UPDATE reservas SET asistencia=$presentismo, pago=$pago, cantLugares=$lugares, anotado=$anotado, contactar=$contactar, contactado=$contactado, rechazar=$rechazar, agenentrevista=$agenentrevista, facebook='$facebook', comentario='$comentario' WHERE idReserva=$id";
			try
			{
				//echo $sql;die;
				$db = Conexion::getConexion('usuarios');
				...
				$c = $e->getCode();
				throw new AppException($sql, 200);
			}
		}
		return true;		
	}
